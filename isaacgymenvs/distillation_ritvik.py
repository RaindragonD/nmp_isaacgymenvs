...
class Dagger:
    ...
    def distill(self):
        ...
        while log_counter < num_iters:
            ...
            even_indices = torch.where(self.env_counter % 1 == 0)[0]
            if self.stereo:
            ...
            # after_aug = time.time()
            # print(f"Time taken for aug: {after_aug - before_aug} seconds")
            ...
            self.ov_env._pred_pos = aux_out["object_pos"]
            ...
            # torch.cuda.synchronize()
            # end_time = time.time()
            # print(f"Time taken for backward step: {end_time - start_time} seconds")
            ...
            # torch.cuda.synchronize()
            # end_step = time.time()
            # print(f"Time taken for env step: {end_step - before_step} seconds")
    ...
    def get_actions(self, obs, policy_type):
        ...
        if policy_type == "student":
            ...
            batch_dict = {
                ...
                "finetune_backbone": obs["finetune_backbone"]
            }
            if "img" in obs:
